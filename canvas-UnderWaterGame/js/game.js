...
class PlayerBullet extends Sprite {
    constructor(x, y, speed) {
        // class Sprite (imageName, x, y)
        super('Bullet_Player_10x40px.png', x, y);
        this.speed = speed;
        this.isExist = true;
...
class EasyEnemy extends Sprite {
    constructor(x, y) {
        super('Bone_Fish_52x78px.png', x, y);
        this.speed = 0.03 + Math.random() * 0.03; // 0.02 ... 0.04
        this.hp = 2;
        this.size = 24;
        this.damage = 12;
        this.isExist = true;
...
// SEA URCHIN
class SeaUrchin extends Sprite {
    constructor(x, y, speed) {
        // class Sprite (imageName, x, y )
        super( 'seaUrchin_167x180px.png', x, y );
...
// функция добавления нового ежа
function addSeaUrchin() {
    ...
    seaUrchinArr.push( new SeaUrchin(xx, yy, speed) );
}
...
function gameLoop(dt) {
    ...
    // обновляем ежей
    for (let i = 0; i < seaUrchinArr.length; i++) seaUrchinArr[i].update(dt);
    seaUrchinArr = getExistsObjectsFromArr(seaUrchinArr);
    if (seaUrchinArr.length < maxSeaUrchins) addSeaUrchin();
